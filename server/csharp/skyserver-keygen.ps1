[CmdletBinding()]
param(
    [string]$OutputDirectory = (Join-Path $PSScriptRoot 'keys'),
    [string]$PatcherOutput = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'patcher\dist\skypatch.ps1'),
...
$templatePath = Join-Path (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'patcher') 'skypatch.ps1'
