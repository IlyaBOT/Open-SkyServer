...
Client-written `u/<login>` contact documents now update the SQL contact membership in the same transaction as the document. The document's `3/10` identity must match the path and refer to an active account; a document uploaded by the real client passed this structural check. This makes document upload and deletion symmetric for server-side contact lists, but does not yet prove that a fresh profile reconstructs its full authorization state from the server.
