...
int main_skyauth4(char *username, char *password) {
    ...
    env_ip = getenv("SKYAUTH_HOST");
    env_port = getenv("SKYAUTH_PORT");

	ip=strdup((env_ip && env_ip[0]) ? env_ip : "91.190.218.40");
	port=(unsigned short)((env_port && env_port[0]) ? atoi(env_port) : 33033);
    printf("Auth server: %s:%u\n", ip, port);
