...
static int append_hex(char *dst, int pos, const char *src) {
...
static u32 SkypePrepareLocalAuthPacket(char *username, char *password, char *login3, u32 *n3) {
	char payload[0x400];
	int payload_len;
	int frame_len;

	payload_len = 0;
	payload_len += sprintf(payload + payload_len, "SKYSERVER-AUTH/1\nuser_hex=");
	payload_len = append_hex(payload, payload_len, username);
	payload_len += sprintf(payload + payload_len, "\npass_hex=");
	payload_len = append_hex(payload, payload_len, password);
	payload_len += sprintf(payload + payload_len, "\n");
	...
	login3[1] = 0x03;
	login3[2] = 0x01;
	login3[3] = (char)((payload_len >> 8) & 0xFF);
	login3[4] = (char)(payload_len & 0xFF);
	memcpy(login3 + 5, payload, payload_len);

	*n3 = frame_len;
	show_memory(login3, *n3, "Before RC4 encrypt local auth");
	RC4_crypt(login3, *n3, &rc4_send, 0);
	show_memory(login3, *n3, "After RC4 encrypt local auth");
...
u32 do_login_process (char *username, char *password, u32 *public_key, u8 *credentials) {
	...
	SkypePrepareLocalAuthPacket(username, password, login3, &n3);

	if (send (sock, login1, n1, 0) != n1) return 0;	// WTF?
	if (send (sock, login2, n2, 0) != n2) return 0;	// WTF?
	if (send (sock, login3, n3, 0) != n3) return 0;	// local community auth frame
