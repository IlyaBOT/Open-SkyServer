...
Current Skype 4.2 package workflow (2026-09-22): see [the deployment guide](../../server/csharp/DEPLOYMENT.md).
`../tools/New-ClientBundle.ps1` packages the verified rebuilt image with a hash-checked installer/restorer and account-scoped directory settings. The limitations below apply to the older constant-patcher template, not that package workflow. A changed server address requires a new signed client build.
