...
The current Git index includes `private-keys`, a working database and a client profile. `.gitignore` does NOT remove existing tracked files or old commits. Do not publish a tag/source archive of this tree. Audit the remote/history, rotate any exposed authority keys and account passwords, and publish from a sanitized source tree. A key rotation requires rebuilding every client package. Never send friends server private keys, databases, logs or profile directories. Use separate beta passwords, not passwords used on any other service.
...
`skyserver/bin/Release` into the server application directory. Install the
...
```powershell
.\skyserver\skyserver-keygen.ps1 -OutputDirectory C:\SkyData\keys `
...
```powershell
$server = '.\skyserver\bin\Release\skyserver.exe'
...
```powershell
py -3 -m pip install --only-binary=:all: --target skyserver/.packages/analysis pefile==2024.8.26 unicorn==2.1.4
py -3 skyserver/tools/Inspect-PackedSkype.py --client C:\CleanSkype\Skype.exe --seconds 120 --output skyserver/diagnostics/release.image.bin
py -3 skyserver/tools/Build-Skype42Lab.py --client C:\CleanSkype\Skype.exe --image skyserver/diagnostics/release.image.bin --keys-dir C:\SkyData\keys --server-ip 203.0.113.10 --output-dir skyserver/diagnostics/release-client
.\skyserver\tools\New-ClientBundle.ps1 -BuildDirectory .\skyserver\diagnostics\release-client -OutputDirectory .\skyserver\dist\release-client
```
...
A Git tag points at a commit; binaries are Release assets. Publication is held
because the current tracked tree contains private keys and account data.
Remove secrets from the publishable tree/history, rotate compromised keys,
rebuild and retest clients, then publish a reviewed tag and attach the ZIP.
No existing commits, profiles or keys were deleted or rewritten by this change.
